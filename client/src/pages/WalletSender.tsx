...
export default function WalletSender() {
  ...
  const filteredPayments = mockSenderPayments.filter(payment => {
    if (statusFilter !== "all" && payment.status !== statusFilter) return false;
    if (searchQuery && !payment.parcelTitle.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !payment.parcelId.toLowerCase().includes(searchQuery.toLowerCase()) &&
        !payment.travelerName.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  }).sort((a, b) => {
    switch (sortBy) {
      case "amount":
  ...
  const totalPaid = mockSenderPayments.filter(p => p.status === "paid").reduce((sum, p) => sum + p.amount + p.fees, 0);
  const inEscrow = mockSenderPayments.filter(p => p.status === "in-escrow").reduce((sum, p) => sum + p.amount, 0);
  const totalRefunded = mockSenderPayments.filter(p => p.status === "refunded").reduce((sum, p) => sum + p.amount, 0);
  ...
  const downloadInvoice = (payment: SenderPayment) => {
    ...
    console.log("Downloading invoice for:", payment.invoiceId);
  };
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-h1 font-bold text-airbar-black">Sender Payments</h1>
            <p className="text-gray-600">Track your payment history for package deliveries</p>
          </div>
        </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-yellow-600">${inEscrow.toFixed(2)}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-red-600">${totalRefunded.toFixed(2)}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-gray-600">${totalFees.toFixed(2)}</p>
                </div>
          ...
          <CardContent className="p-6">
            <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0">
              ...
              <div className="text-sm text-gray-600">
                {filteredPayments.length} of {mockSenderPayments.length} payments
              </div>
            </div>
          </CardContent>
        </Card>
                    ...
                    <div className="flex items-start space-x-4">
                      ...
                      <div>
                        <div className="flex items-center space-x-2 mb-2">
                          <h3 className="font-semibold text-lg">{payment.parcelTitle}</h3>
                        ...
                        <p className="text-sm text-gray-600 mb-2">{payment.parcelDescription}</p>
                        <p className="text-xs text-gray-500 mb-3">
                          Parcel ID: {payment.parcelId} • Invoice: {payment.invoiceId}
                        </p>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4 text-sm">
                    ...
                    {payment.deliveryDate && (
                      <div className="flex items-center space-x-2">
                        ...
                        <span>Delivered: {new Date(payment.deliveryDate).toLocaleDateString()}</span>
                      </div>
                    )}
                    ...
                    {payment.refundDate && (
                      <div className="flex items-center space-x-2">
                        ...
                        <span>Refunded: {new Date(payment.refundDate).toLocaleDateString()}</span>
                      </div>
                    )}
            ...
            <div className="flex items-start space-x-3">
              ...
              <div className="text-sm text-blue-800">
                ...
                <ul className="space-y-1">
                  <li>• Payments are charged when your package is picked up by the traveler</li>
                  <li>• Funds are held in escrow until delivery is confirmed</li>
                  <li>• Refunds are processed automatically if delivery fails</li>
