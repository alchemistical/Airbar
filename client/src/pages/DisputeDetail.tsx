...
import { AlertCircle, ArrowLeft, Camera, CheckCircle, Clock, FileText, MessageSquare, Send, Shield, User, AlertTriangle, DollarSign, X } from "lucide-react";
...
export default function DisputeDetail() {
  ...
  const confirmResolution = () => {
    addTimelineMutation.mutate({
      actor: `User ${userId}`,
      actorRole: userId === dispute?.senderId ? "sender" : "traveler",
      type: resolutionAction === "accept" ? "resolution_accepted" : "resolution_declined",
  ...
  const getTimelineIcon = (type: string) => {
    switch (type) {
      ...
      case "resolution_accepted": return <CheckCircle className="h-4 w-4" />;
      case "resolution_declined": return <X className="h-4 w-4" />;
  ...
  const deadlineStatus = () => {
    ...
    if (now > resolutionDue) {
      return { text: "Resolution overdue", color: "text-red-600", icon: AlertTriangle };
    } else if (now > firstReplyDue && dispute.status === "open") {
      return { text: "Reply overdue", color: "text-orange-600", icon: Clock };
    } else {
      const nextDeadline = dispute.status === "open" ? firstReplyDue : resolutionDue;
      return { 
    ...
    <div className="space-y-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Match #{dispute.matchId} • Reported {formatDistanceToNow(new Date(dispute.createdAt))} ago
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div>
                <p className="text-sm text-gray-600 mb-1">Preferred Resolution</p>
                <p className="font-medium capitalize">{dispute.preferredOutcome}</p>
              </div>
                  ...
                  <div key={index} className="flex gap-3">
                    <div className="relative">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        entry.actorRole === "support" ? "bg-purple-100" :
                        entry.actorRole === "sender" ? "bg-blue-100" : "bg-green-100"
                      }`}>
                      ...
                      {entry.type === "offer" && entry.payload && (
                        <Card className="mt-2 bg-purple-50 border-purple-200">
                          <CardContent className="p-4">
                            <p className="font-medium text-sm mb-2">Resolution Offer</p>
                            <p className="text-sm">{entry.payload.description}</p>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-end">
                  <Button
                    onClick={handleReply}
                    disabled={!replyText.trim() || addTimelineMutation.isPending}
          ...
          {dispute.status === "offer" && hasOffer && (
            <Card className="border-purple-200">
              <CardHeader className="bg-purple-50">
                <CardTitle className="text-lg">Resolution Offer Available</CardTitle>
              </CardHeader>
            ...
            <CardContent className="space-y-3">
              ...
              <div>
                ...
                <p className="font-medium text-sm">
                  {format(new Date(dispute.firstReplyDue), "MMM d, yyyy h:mm a")}
                </p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Resolution Due</p>
                <p className="font-medium text-sm">
                  {format(new Date(dispute.resolutionDue), "MMM d, yyyy h:mm a")}
                </p>
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600">User #{dispute.senderId}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600">User #{dispute.travelerId}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
      ...
      <Dialog open={showResolutionModal} onOpenChange={setShowResolutionModal}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              {resolutionAction === "accept" ? "Accept Resolution Offer" : "Decline Resolution Offer"}
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              className={resolutionAction === "accept" 
