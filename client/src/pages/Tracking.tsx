...
type TrackingStatus = "confirmed" | "picked-up" | "in-transit" | "delivered" | "cancelled";
...
export default function Tracking() {
  ...
  const mockTrackingData: TrackingItem[] = [
    {
      ...
      timeline: {
        confirmed: { date: "2025-01-08", time: "09:15", note: "Match confirmed by both parties" },
        pickedUp: { date: "2025-01-09", time: "11:30", note: "Package collected from sender" },
        inTransit: { date: "2025-01-10", time: "14:30", note: "Package on flight to San Francisco" }
      },
      hasIssue: false,
      isDelayed: false,
      requiresId: true
    },
    {
      ...
      timeline: {
        confirmed: { date: "2025-01-09", time: "14:20", note: "Match confirmed" },
        pickedUp: { date: "2025-01-10", time: "16:45", note: "Package received in good condition" }
      },
      hasIssue: false,
      isDelayed: false,
      requiresId: false
    },
    {
      ...
      timeline: {
        confirmed: { date: "2025-01-08", time: "20:30", note: "Urgent delivery confirmed" },
        pickedUp: { date: "2025-01-09", time: "06:00", note: "Express pickup completed" },
        inTransit: { date: "2025-01-09", time: "08:15", note: "Priority handling" },
        delivered: { date: "2025-01-09", time: "18:00", note: "Delivered to hospital reception" }
      },
      hasIssue: false,
      isDelayed: false,
      requiresId: true
  ...
  const filteredData = mockTrackingData.filter(item => {
    // Status filter
    if (statusFilter === "in-progress" && !["confirmed", "picked-up", "in-transit"].includes(item.status)) return false;
    ...
    if (searchQuery && !item.parcelTitle.toLowerCase().includes(searchQuery.toLowerCase()) &&
        ...
        !item.otherUserName.toLowerCase().includes(searchQuery.toLowerCase())) return false;
  ...
  const getStatusColor = (status: TrackingStatus, hasIssue: boolean, isDelayed: boolean) => {
      ...
      <div className="space-y-6">
        <div className="relative">
          {steps.map((step, index) => {
            const timelineItem = item.timeline[step.key as keyof typeof item.timeline];
            const isCompleted = !!timelineItem;
            const isCurrent = !isCompleted && index === steps.findIndex(s => !item.timeline[s.key as keyof typeof item.timeline]);
      ...
      <CardContent className="p-6">
        <div className="flex items-start justify-between mb-4">
          <div>
            <div className="flex items-center space-x-2 mb-2">
              ...
              <Badge className={getStatusColor(item.status, item.hasIssue, item.isDelayed)}>
            ...
            <p className="text-sm text-gray-600 mb-2">Match ID: {item.matchId}</p>
            <div className="flex items-center space-x-4 text-sm text-gray-600">
              <div className="flex items-center space-x-1">
                <User className="h-4 w-4" />
                <span>{activeRole === "traveler" ? "Sender" : "Traveler"}: {item.otherUserName}</span>
                {item.otherUserIsKyc && <CheckCircle className="h-3 w-3 text-green-500" />}
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm" onClick={() => setSelectedTracking(item)}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div className="flex items-center space-x-2 text-sm">
            ...
            <span>Due: {new Date(item.deliveryDeadline).toLocaleDateString()}</span>
          </div>
          <div className="flex items-center space-x-2 text-sm">
  ...
  return (
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-h1 font-bold text-airbar-black">Package Tracking</h1>
            <p className="text-gray-600">Monitor your parcel deliveries in real-time</p>
          </div>
        </div>
              ...
              <>
                <DialogHeader>
                  <DialogTitle className="flex items-center space-x-2">
                    ...
                    <Badge className={getStatusColor(selectedTracking.status, selectedTracking.hasIssue, selectedTracking.isDelayed)}>
                  ...
                  <div className="space-y-6">
                    <div>
                      ...
                      <div className="space-y-3 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{selectedTracking.matchId}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Description:</span>
                          <span className="font-medium text-right">{selectedTracking.parcelDescription}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Weight:</span>
                          <span className="font-medium">{selectedTracking.weight}kg</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Dimensions:</span>
                          <span className="font-medium">{selectedTracking.dimensions}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Value:</span>
                          <span className="font-medium">${selectedTracking.value}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Reward:</span>
                          <span className="font-medium text-green-600">${selectedTracking.rewardAmount}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">ID Required:</span>
                          <span className="font-medium">{selectedTracking.requiresId ? "Yes" : "No"}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      <h3 className="text-lg font-semibold mb-4">
                        {activeRole === "traveler" ? "Sender" : "Traveler"} Details
                      </h3>
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            <div className="flex items-center space-x-2">
                              <span className="font-medium">{selectedTracking.otherUserName}</span>
                            ...
                            <div className="flex items-center space-x-1 text-sm text-gray-600">
                              ...
                              <span>{selectedTracking.otherUserRating} rating</span>
                            </div>
                          </div>
                        </div>
                    ...
                    <div>
                      ...
                      <div className="space-y-4 text-sm">
                        <div>
                          ...
                          <p className="font-medium">{selectedTracking.pickupAddress}</p>
                        </div>
                        <div>
                          <p className="text-gray-600 mb-1">Delivery Address:</p>
                          <p className="font-medium">{selectedTracking.deliveryAddress}</p>
                        </div>
                        <div>
                          <p className="text-gray-600 mb-1">Delivery Deadline:</p>
                          <p className="font-medium">{new Date(selectedTracking.deliveryDeadline).toLocaleDateString()}</p>
                        </div>
                      </div>
                    </div>
                  </div>
