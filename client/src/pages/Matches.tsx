...
export default function Matches() {
  ...
  const getNextStatusAction = (status: string, role: RoleFilter) => {
    if (role === "traveler") {
      switch (status) {
        case "pending": return { text: "Mark as Picked Up", action: "pick-up" };
        case "picked-up": return { text: "Mark as In Transit", action: "in-transit" };
        case "in-transit": return { text: "Mark as Delivered", action: "delivered" };
        default: return null;
      }
    } else {
      switch (status) {
        case "delivered": return { text: "Confirm Delivery", action: "confirm" };
        default: return null;
      }
    }
  };
  ...
  const getProgressSteps = (status: string) => {
    const steps = ["Pending", "Picked Up", "In Transit", "Delivered", "Confirmed"];
    const currentIndex = steps.findIndex(step => 
      step.toLowerCase().replace(" ", "-") === status
    );
  ...
  const filteredMatches = matches.filter(match => {
    const matchesSearch = match.parcelDescription.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesStatus = statusFilter === "all" || 
                         (statusFilter === "in-progress" && ["picked-up", "in-transit"].includes(match.status)) ||
                         (statusFilter === "completed" && ["delivered", "confirmed"].includes(match.status)) ||
  ...
  const handleReportIssue = (matchId: number) => {
    // In real app, this would make an API call
    console.log(`Reporting issue for match ${matchId}:`, issueReason, issueDescription);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="grid grid-cols-4 gap-4">
            {[
              ...
              { label: "Pending", count: matches.filter(m => m.status === "pending").length, status: "pending" },
              { label: "In Progress", count: matches.filter(m => ["picked-up", "in-transit"].includes(m.status)).length, status: "in-progress" },
              { label: "Completed", count: matches.filter(m => ["delivered", "confirmed"].includes(m.status)).length, status: "completed" }
            ].map((stat) => (
              <Card 
                key={stat.status} 
              ...
              >
                <CardContent className="p-3 text-center">
                  <div className="text-h4 font-bold text-airbar-black">{stat.count}</div>
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-h3 font-semibold text-gray-900 mb-2">
                {searchTerm || statusFilter !== "all" ? "No matching deliveries" : "No matches yet"}
              </h3>
                ...
                <CardContent className="p-6">
                  <div className="flex items-start justify-between">
                    ...
                    <div className="flex-1 space-y-4">
                        ...
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-small text-gray-600">
                              Match ID: #{match.id} • {match.parcelSize} • {match.deliveryType}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback className="bg-blue-100 text-blue-600">
                            {match.otherUserName.split(' ').map(n => n[0]).join('')}
                          </AvatarFallback>
                        </Avatar>
                        <div className="flex-1">
                          <div className="flex items-center space-x-2">
                            ...
                            <div className="flex items-center space-x-1">
                              ...
                              <span className="text-xs text-gray-600">{match.otherUserRating}</span>
                            </div>
                      ...
                      <div className="space-y-2">
                        <div className="flex justify-between text-xs text-gray-600">
                          {getProgressSteps(match.status).map((step, index) => (
                            <span key={index} className={step.completed ? "text-blue-600 font-medium" : ""}>
                              {step.name}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-small">
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-gray-600">Weight: {match.parcelWeight}kg</span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Clock className="h-4 w-4 text-gray-400" />
                          <span className="text-gray-600">Updated: {formatDate(match.lastUpdated)}</span>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Package className="h-4 w-4 text-gray-400" />
                          <span className="text-gray-600">Size: {match.parcelSize}</span>
                        </div>
                        <div>
                          <span className="text-gray-600">Reward: </span>
                          <span className="font-semibold text-green-600">${match.rewardAmount}</span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col space-y-2 ml-6">
                      ...
                      {(() => {
                        const nextAction = getNextStatusAction(match.status, roleFilter);
                        if (nextAction) {
                          return (
                            <Button 
                              size="sm" 
                              ...
                              onClick={() => {
                                ...
                                if (nextAction.action === "delivered") {
                                  ...
                                  handleStatusUpdate(match.id, nextAction.action);
                                }
                              }}
                            >
            ...
            {selectedMatch && (
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label>Parcel</Label>
                    <p className="text-sm text-gray-600">{selectedMatch.parcelDescription}</p>
                  </div>
                  <div>
                    <Label>Route</Label>
                    <p className="text-sm text-gray-600">{selectedMatch.fromCity} → {selectedMatch.toCity}</p>
                  </div>
                  <div>
                    <Label>Pickup Address</Label>
                    <p className="text-sm text-gray-600">{selectedMatch.pickupAddress}</p>
                  </div>
                  <div>
                    <Label>Delivery Address</Label>
                    <p className="text-sm text-gray-600">{selectedMatch.deliveryAddress}</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-2">
                <Button 
                  onClick={() => selectedMatch && handleStatusUpdate(selectedMatch.id, "delivered")}
                  className="flex-1"
                >
                  Mark as Delivered
                </Button>
        ...
        <Dialog open={showIssueModal} onOpenChange={setShowIssueModal}>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div className="flex space-x-2">
                <Button 
                  onClick={() => selectedMatch && handleReportIssue(selectedMatch.id)}
                  variant="destructive"
                  className="flex-1"
                >
                  Report Issue
