...
export default function HistoryDetail() {
  ...
  const mockRecord: HistoryDetailRecord = {
    ...
    packageDescription: "High-end laptop and accessories for business meeting presentation. Includes charger, mouse, and presentation materials.",
    fromCity: "New York",
    toCity: "Miami",
    ...
    hasDispute: false,
    archived: false,
    specialInstructions: "Handle with care - fragile electronics. Recipient will be available from 2-6 PM.",
  ...
  const getRoleColor = (role: string) => {
    return role === "traveler" ? "bg-blue-100 text-blue-800" : "bg-indigo-100 text-indigo-800";
  };
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-h1 font-bold text-airbar-black">{mockRecord.packageTitle}</h1>
              <p className="text-gray-600">Match ID: {mockRecord.matchId}</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            <Badge variant="secondary" className={getRoleColor(mockRecord.userRole)}>
              {mockRecord.userRole === "traveler" ? "As Traveler" : "As Sender"}
            </Badge>
            <Badge variant="secondary" className={getStatusColor(mockRecord.status)}>
              {mockRecord.status.charAt(0).toUpperCase() + mockRecord.status.slice(1)}
            </Badge>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <h3 className="font-semibold text-lg mb-2">{mockRecord.packageTitle}</h3>
                  <p className="text-gray-600 mb-4">{mockRecord.packageDescription}</p>
                </div>
                ...
                {mockRecord.specialInstructions && (
                  <div>
                    ...
                    <p className="text-sm bg-gray-50 p-3 rounded">{mockRecord.specialInstructions}</p>
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{mockRecord.fromAddress}</p>
                    <p className="text-xs text-gray-500 mt-1">Window: {mockRecord.pickupWindow}</p>
                  </div>
                  <div>
                    <h4 className="font-medium mb-2 flex items-center">
                    ...
                    <p className="text-sm text-gray-600">{mockRecord.toAddress}</p>
                    <p className="text-xs text-gray-500 mt-1">Window: {mockRecord.deliveryWindow}</p>
                  </div>
                </div>
                ...
                <CardContent>
                  <div className="bg-orange-50 p-4 rounded-lg">
                    <p className="text-sm text-orange-800">
                      {mockRecord.disputeReason || "Dispute was raised regarding delivery conditions. Issue has been resolved through customer support."}
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              <CardContent>
                <div className="text-center">
                  <p className="text-sm text-gray-500 mb-1">
                    {mockRecord.userRole === "traveler" ? "Amount Earned" : "Amount Paid"}
                  </p>
                  <p className={`text-3xl font-bold ${
                    mockRecord.userRole === "traveler" ? "text-green-600" : "text-blue-600"
                  }`}>
                    {mockRecord.userRole === "traveler" ? "+" : "-"}${mockRecord.rewardAmount.toFixed(2)}
                  </p>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center">
                  ...
                  {mockRecord.userRole === "traveler" ? "Sender" : "Traveler"} Information
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-medium">{mockRecord.otherUserName}</span>
                  <div className="flex items-center">
                    <Star className="h-4 w-4 text-yellow-500 mr-1" />
                    <span className="text-sm">{mockRecord.otherUserRating}</span>
                  </div>
                </div>
