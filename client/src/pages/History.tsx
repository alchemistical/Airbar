...
export default function History() {
  ...
  const filteredRecords = mockHistoryRecords.filter(record => {
    ...
    const matchesDate = !dateRange?.from || new Date(record.completedDate) >= dateRange.from;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-h1 font-bold text-airbar-black">Delivery History</h1>
            <p className="text-gray-600">Complete record of your delivery activity and transactions</p>
          </div>
          ...
          <CardContent>
            {filteredRecords.length === 0 ? (
              <div className="text-center py-12 text-gray-500">
                ...
                <h3 className="text-lg font-medium mb-2">No history records found</h3>
              ...
              <div className={viewMode === "cards" ? "grid grid-cols-1 md:grid-cols-2 gap-6" : "space-y-4"}>
                {filteredRecords.map((record) => (
                  <div key={record.id} className={`border rounded-lg p-6 hover:shadow-md transition-shadow ${
                    ...
                    <div className={viewMode === "cards" ? "space-y-4" : "flex-1 grid grid-cols-6 gap-4 items-center"}>
                      {/* Header */}
                      <div className={viewMode === "cards" ? "flex items-start justify-between" : "col-span-2"}>
                        <div>
                          <h3 className="font-semibold text-lg">{record.packageTitle}</h3>
                          <p className="text-sm text-gray-600">{record.matchId}</p>
                          {viewMode === "cards" && (
                            <p className="text-sm text-gray-500 mt-1">{record.packageDescription}</p>
                          )}
                        </div>
                      ...
                      {/* Route */}
                      <div className={viewMode === "cards" ? "flex items-center space-x-2 text-gray-600" : ""}>
                      ...
                      <div className={viewMode === "list" ? "" : "flex items-center justify-between"}>
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="text-sm">{record.otherUserName}</span>
                          <span className="text-xs text-gray-500">★ {record.otherUserRating}</span>
                        </div>
                        {viewMode === "cards" && (
                          <Badge variant="secondary" className={getRoleColor(record.userRole)}>
                            {record.userRole === "traveler" ? "As Traveler" : "As Sender"}
                          </Badge>
                        )}
                      </div>

                      {/* Details */}
                      <div className={viewMode === "list" ? "text-sm text-gray-600" : "text-sm text-gray-600 space-y-1"}>
                        <div>Weight: {record.weight}kg • Size: {record.size}</div>
                        {viewMode === "cards" && (
                      ...
                      <div className={viewMode === "list" ? "text-right" : "flex items-center justify-between"}>
                        <div>
                          <p className="font-semibold text-lg">
                            {record.userRole === "traveler" ? "+" : "-"}${record.rewardAmount.toFixed(2)}
                          </p>
                          {viewMode === "list" && <p className="text-xs text-gray-500">{record.completedDate}</p>}
                        </div>
                      ...
                      {viewMode === "list" && (
                        <div>
                          <Badge variant="secondary" className={getRoleColor(record.userRole)}>
                            {record.userRole === "traveler" ? "Traveler" : "Sender"}
                          </Badge>
                        </div>
                      )}
                    </div>
                    ...
                    {/* Actions */}
                    <div className={viewMode === "cards" ? "flex items-center justify-between pt-4 border-t" : "ml-4"}>
                      {viewMode === "cards" && record.hasDispute && (
                        <Badge variant="secondary" className="bg-orange-100 text-orange-800">
                          Dispute
                        </Badge>
