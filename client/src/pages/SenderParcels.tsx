...
export default function SenderParcels() {
  ...
  const filteredParcels = senderParcels.filter(parcel => {
    const matchesSearch = parcel.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const matchesStatus = statusFilter === "all" || parcel.status.toLowerCase() === statusFilter;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
        {[
          ...
          { label: "Waiting", count: senderParcels.filter(p => p.status === "waiting").length, status: "waiting" },
          { label: "Matched", count: senderParcels.filter(p => p.status === "matched").length, status: "matched" },
          { label: "In Transit", count: senderParcels.filter(p => p.status === "in-transit").length, status: "in-transit" },
          { label: "Delivered", count: senderParcels.filter(p => p.status === "delivered").length, status: "delivered" }
        ].map((stat) => (
          <Card 
            key={stat.status} 
          ...
          >
            <CardContent className="p-4 text-center">
              <div className="text-h2 font-bold text-airbar-black">{stat.count}</div>
        ...
        <Card>
          <CardContent className="p-12 text-center">
            ...
            <h3 className="text-h3 font-semibold text-gray-900 mb-2">
              {searchTerm || statusFilter !== "all" ? "No matching parcels" : "No parcels sent yet"}
            </h3>
              ...
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex-1 space-y-4">
                      ...
                      <div className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                        <Avatar className="h-8 w-8">
                          <AvatarFallback className="bg-blue-100 text-blue-600">
                            {parcel.travelerName.split(' ').map(n => n[0]).join('')}
                          </AvatarFallback>
                        </Avatar>
                        <div className="flex-1">
                          <div className="flex items-center space-x-2">
                            ...
                            <div className="flex items-center space-x-1">
                              ...
                              <span className="text-xs text-gray-600">{parcel.travelerRating}</span>
                            </div>
                          </div>
                          <p className="text-xs text-gray-600">{parcel.tripRoute} • {parcel.tripDate}</p>
                        </div>
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 text-small">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-gray-600">Deadline: {formatDate(parcel.deadline)}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Calendar className="h-4 w-4 text-gray-400" />
                        <span className="text-gray-600">Created: {formatDate(parcel.createdAt)}</span>
                      </div>
                      <div>
                        <span className="text-gray-600">Reward: </span>
                        <span className="font-semibold text-green-600">${parcel.rewardAmount}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col space-y-2 ml-6">
                    ...
                    {(parcel.status === "matched" || parcel.status === "in-transit") && (
                    ...
                    {parcel.status === "waiting" && (
                      <Button variant="outline" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50 w-full">
