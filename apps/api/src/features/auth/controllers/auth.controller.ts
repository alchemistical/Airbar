...
export class AuthController {
    ...
    try {
      ...
      const newUser = await db
        ...
        .values({
          email,
          username,
          password: passwordHash, // Map to both password and passwordHash
          passwordHash,
      ...
      res.status(201).json({
        ...
        data: {
          user: {
            ...
            role: newUser[0].role || 'TRAVELER',
            isKYCVerified: newUser[0].isKYCVerified || false,
            emailVerified: newUser[0].emailVerified || false,
  ...
  static async login(req: Request, res: Response): Promise<void> {
    try {
      ...
      // Verify password
      const passwordToVerify = foundUser.passwordHash || foundUser.password;
      if (!passwordToVerify) {
        res.status(500).json({
          success: false,
          error: {
      ...
      const isValidPassword = await JWTService.verifyPassword(password, passwordToVerify);

      if (!isValidPassword) {
        // Increment failed attempts
        const currentAttempts = foundUser.failedLoginAttempts || 0;
        await db
          .update(users)
          .set({
  ...
  static async getUserSessions(req: AuthRequest, res: Response): Promise<void> {
    try {
      ...
      const sessionsWithCurrent = userSessions.map((session: any) => ({
        ...session,
