import {
  ...
  type InsertUser,
  type InsertTrip,
  type InsertParcelRequest,
  type InsertNotification,
  type InsertDispute,
  type InsertMatchRequest,
  type InsertMatch,
  type DashboardMetrics,
  type TripWithRequests,
  type ParcelRequestWithSender,
} from "@airbar/shared/schema";
...
export interface IStorage {
  ...
  // Discovery methods
  discoverTravelersForSenders(params: {
    origin: string;
    destination: string;
  ...
  discoverPackagesForTravelers(params: {
    origin: string;
    destination: string;
    dateRange?: { start: string; end: string };
    filters?: any;
...
export class MemStorage implements IStorage {
  ...
  private initializeData() {
    ...
    const dispute1: Dispute = {
      ...
      reason: "damaged",
      description:
        "The package arrived with significant damage. The electronic device inside was broken and the packaging was torn. I have photos showing the condition.",
      preferredOutcome: "refund",
      evidence: [
        {
          url: "/sample/damage1.jpg",
          type: "image/jpeg",
          uploadedAt: "2025-01-10T10:00:00Z",
        },
        {
          url: "/sample/damage2.jpg",
          type: "image/jpeg",
          uploadedAt: "2025-01-10T10:01:00Z",
        },
      ],
      timeline: [
        {
    ...
    const dispute2: Dispute = {
      ...
      reason: "late",
      description:
        "Package was delivered 3 days after the agreed delivery date. This caused significant inconvenience as it was meant for a birthday gift.",
      ...
      timeline: [
        ...
        },
        {
          ...
          type: "reply",
          message:
            "I apologize for the delay. There were unexpected travel disruptions due to weather.",
        },
        {
  ...
  async getDashboardMetrics(userId: number): Promise<DashboardMetrics> {
    ...
    const userEarnings = Array.from(this.earnings.values()).filter(
      e => e.userId === userId
    );
  ...
  async getParcelRequestsForUser(
    userId: number
  ): Promise<ParcelRequestWithSender[]> {
  ...
  async getUserMatchRequests(userId: number): Promise<MatchRequest[]> {
    return Array.from(this.matchRequests.values())
      .filter(
        request => request.senderId === userId || request.travelerId === userId
      )
      .sort(
        (a, b) =>
          new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime()
  ...
  async updateMatchRequestStatus(
    id: number,
    status: string,
    acceptedAt?: Date
  ): Promise<MatchRequest> {
  ...
  async discoverTravelersForSenders(params: {
    origin: string;
    destination: string;
    dateRange?: { start: string; end: string };
    filters?: any;
  }): Promise<any[]> {
    const { origin, destination, dateRange, filters } = params;

    // Find trips that match the route criteria
    let availableTrips = Array.from(this.trips.values()).filter(trip => {
      // Simple string matching for demo - in production would use proper geocoding
      const routeMatches =
        trip.fromCity.toLowerCase().includes(origin.toLowerCase()) &&
        trip.toCity.toLowerCase().includes(destination.toLowerCase());
      ...
      if (dateRange && dateRange.start && dateRange.end) {
        ...
        dateInRange = tripDate >= startDate && tripDate <= endDate;
      }

      return routeMatches && isActive && dateInRange;
    });

    // Apply filters if provided
    if (filters) {
      if (filters.maxPrice) {
        availableTrips = availableTrips.filter(
          trip => parseFloat(trip.pricePerKg || "0") <= filters.maxPrice
        );
      }
      ...
      if (filters.minTrustScore) {
        availableTrips = availableTrips.filter(trip => {
          const user = this.users.get(trip.userId);
          return parseFloat(user?.rating || "0") >= filters.minTrustScore;
        });
      }
    }

    // Transform trips into match format
    return availableTrips.map(trip => {
      const traveler = this.users.get(trip.userId);
      return {
        id: trip.id,
        type: "travel_profile",
        user: {
          ...
          name: traveler
            ...
            : "Unknown",
          trustScore: parseFloat(traveler?.rating || "0"),
          verified: traveler?.isKycVerified || false,
        ...
        price: parseFloat(trip.pricePerKg || "0"),
        capacity: `${trip.maxWeight}kg`,
        travelDate: trip.departureDate.toISOString(),
        matchScore: 0.85 + Math.random() * 0.1, // Mock match score
      };
    });
  }

  async discoverPackagesForTravelers(params: {
    origin: string;
    destination: string;
    dateRange?: { start: string; end: string };
    filters?: any;
    ...
    // Find parcel requests that match the route criteria
    let availablePackages = Array.from(this.parcelRequests.values()).filter(
      request => {
        // Simple string matching for demo - in production would use proper geocoding
        const routeMatches =
          request.fromCity.toLowerCase().includes(origin.toLowerCase()) &&
          request.toCity.toLowerCase().includes(destination.toLowerCase());

        const isPending = request.status === "pending";

        let dateInRange = true;
        if (dateRange && dateRange.start && dateRange.end) {
          const collectionDate = new Date(request.collectionDate);
          const startDate = new Date(dateRange.start);
          const endDate = new Date(dateRange.end);
          dateInRange =
        ...
        return routeMatches && isPending && dateInRange;
      }
    );
    ...
    if (filters) {
      if (filters.minPrice) {
        availablePackages = availablePackages.filter(
          request => parseFloat(request.offerAmount || "0") >= filters.minPrice
        );
      }
      if (filters.maxWeight) {
        availablePackages = availablePackages.filter(
          request => parseFloat(request.weight) <= filters.maxWeight
        );
      }
      if (filters.verifiedOnly) {
        availablePackages = availablePackages.filter(request => {
          const sender = this.users.get(request.senderId);
          return sender?.isKycVerified;
        });
      }
      if (filters.minTrustScore) {
        availablePackages = availablePackages.filter(request => {
          const sender = this.users.get(request.senderId);
          return parseFloat(sender?.rating || "0") >= filters.minTrustScore;
        });
      }
    }

    // Transform packages into match format
    return availablePackages.map(request => {
      const sender = this.users.get(request.senderId);
      return {
        id: request.id,
        ...
        user: {
          ...
          trustScore: parseFloat(sender?.rating || "0"),
          verified: sender?.isKycVerified || false,
        ...
        price: parseFloat(request.offerAmount || "0"),
        capacity: `${request.weight}kg`,
        travelDate: request.collectionDate.toISOString(),
        matchScore: 0.8 + Math.random() * 0.15, // Mock match score
